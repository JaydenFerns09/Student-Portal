...
app.post("/changedetails", function(req, res){
  ...
  if (adhaarNumber != '')
  {
    Admission.updateOne({password: loginDetails.password},{$set: {adhaarNumber: adhaarNumber}}, function(err){
      if(err){
        console.log(err);
      }
    })
...
app.post("/changemarks", function(req, res){
  ...
  if (flag == 0){
    ...
    if(promsem != undefined && promusn != undefined){
      ...
      if (temp_flag == 0){
        tempsem = chsem;
        Mark.findOne({sem: chsem}, function(err, foundItem){
          if(err){
            console.log(err);
          }else{
            active = true;
